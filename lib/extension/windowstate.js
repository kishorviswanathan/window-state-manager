...
export class WindowStates {
    ...
    constructor(states) {
        if (states != null) {
            Logger.debug('Initializing WindowStates with given state', states);
    ...
    getState() {
        const statesJSON = JSON.stringify(this.windowStates);
        Logger.debug('Window states converted to JSON:', statesJSON)
        return statesJSON;
    }
    ...
    saveWindowPositions() {
        ...
        for (const window of this._getWindows())
            windowId__state.set(window.get_id(), WindowUtils.getState(window));
    }
    ...
    restoreWindowPositions() {
        ...
        for (const window of this._getWindows()) {
            if (windowId__state.has(window.get_id())) {
                const state = windowId__state.get(window.get_id());
                WindowUtils.restoreState(window, state);
            } else
